...
resulttimers = []
msglengtharray = []
msglength = [50,100,150,202,218]
for m in msglength:    
    randomstr = randomString(m-len(str(TESTLENGTH)))
    msglengtharray.append(sys.getsizeof(randomstr))
    counter = TESTLENGTH
    signeddata = []
    ...
    assert check
    hashtimerresult = sum(timers)/len(timers)
    ...
    for k in range (0,TESTLENGTH):
        ...
        timers.append(end-start)
    ecdsatimerresult = sum(timers)/len(timers)
    resulttimers.append([hashtimerresult*1000,ecdsatimerresult*1000])

labelList = [] #initialize list for legend names
labelList.append("Hash-based signatures")
labelList.append("ECDSA")


plt.rc('text', usetex=True)
plt.rc('font', family='serif', size='10')
fig, ax1 = plt.subplots(figsize=(4.4, 2.4)) #set for IEEE
ax1.plot(msglength,resulttimers,linewidth=1)
ax1.set_xlabel('Message length (bytes)')
ax1.set_ylabel('Time to verify (msec)')
ax1.set_yscale('log')
plt.legend(labelList,bbox_to_anchor=(0.86,0.99), loc=1,fancybox=True, framealpha=0.0)
plt.tight_layout() #prevent cut-off labels
plt.show()
#plt.savefig('hash-vs-ecdsa-msglength.pgf')  